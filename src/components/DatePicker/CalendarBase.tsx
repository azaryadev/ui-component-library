...
const CalendarBase = forwardRef<HTMLDivElement, CalendarBaseProps>(
  (props, ref) => {
    ...
    const focusOnNextFocusableDay = (
      ...
      const dayToFocus = daysRefs.current[monthIndex]?.[rowIndex]?.[cellIndex]; // Optional chaining
      ...
      switch (event.key) {
        ...
        case "ArrowRight": {
          ...
          if (isNotLastCell) {
            ...
            const nextMonth = daysRefs.current[monthIndex + 1];
            const nextRow = nextMonth?.[payload.rowIndex];
            if (nextRow?.[0]) {
              nextRow[0].focus();
            }
          }
        ...
        case "ArrowLeft": {
          ...
          if (payload.cellIndex !== 0) {
            ...
            const previousMonthDays = daysRefs.current[monthIndex - 1];
            if (previousMonthDays && previousMonthDays[payload.rowIndex]) {
                const previousDay = previousMonthDays[payload.rowIndex]?.[daysPerRow];
                if (previousDay) {
                    previousDay.focus();
                }
            }
