...
const Upload = forwardRef<HTMLDivElement, UploadProps>((props, ref) => {
  ...
  const filesToArray = (files: File[]) =>
    Object.keys(files)
      ...
      .filter((file): file is File => file !== undefined); // Type guard to ensure only File objects
  ...
  const onNewFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (result) {
      const updatedFiles = addNewFiles(newFiles).filter(
        (file): file is File => file !== undefined
      ); // Ensure strict File[]
      setFiles(updatedFiles); // No more type error
